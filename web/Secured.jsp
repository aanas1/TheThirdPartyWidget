...
<%
    ...
    if (authString != null) {
        if (authString.equalsIgnoreCase("req")) {
            ...
            json.put("x", points.get(0).getX());
            json.put("y", points.get(0).getY());
            json.put("r", points.get(0).getId());
            ...
            for (i = 0; i < ((ArrayList<Point>) session.getAttribute("Points")).size(); i++) {
                if ((((ArrayList<Point>) session.getAttribute("Points")).get(i).isVisited() == false)) {
                    if (((ArrayList<Point>) session.getAttribute("Points")).get(i).getId().equalsIgnoreCase(pointId)) {
                        ...
                        if ((i+1<Utility.getPoints(session).size())){
                         JSONObject json = new JSONObject();
                        json.put("len", 1); 
                        
                        json.put("x", ((ArrayList<Point>) session.getAttribute("Points")).get(i + 1).getX());
                        json.put("y", ((ArrayList<Point>) session.getAttribute("Points")).get(i + 1).getY());
                        json.put("r", ((ArrayList<Point>) session.getAttribute("Points")).get(i + 1).getId());
                        out.print(json);
                        System.out.println(json);
                        out.flush();}
